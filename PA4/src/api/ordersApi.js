...
for (var ct = 0; ct  < orderCount; ct++) {
    ...
    var itemFound = JSON.parse(localStorage.getItem(itemStr));
    console.log("ITEM VALUE " + itemFound);
...
if (orderSummary != null) {
    var vendorSummary = orderNew.itemSummary;
    var htmlStr = orderSummary.innerHTML;
    var total = 0;
    console.log(vendorSummary);
    for (var j = 0; j < vendorSummary.length; j++) {
        var vendor1 = vendorSummary[j];
        htmlStr += "<tr><td>" + vendor1.name + "</td><td>" + vendor1.qty + "</td></tr>";
        total += vendor1.qty;
    }
...
if (radioForm != null) {
    var radios = radioForm.elements["myRadio"];
...
if (shoppingcart != null) {
    var htmlStrSC = shoppingcart.innerHTML;
    ...
    for (var x = 0; x < vendors.length; x++) {
        ...
        for (var y = 0; y < itemList.length; y++) {
            var itemIn = itemList[y];
            listBuilder += "<tr><td class = 'urllist'>" + itemIn.url + "</td></tr>";
        }

        htmlStrSC += headerBuilder + listBuilder;
...
if (orderHistory != null) {
    htmlStr = orderHistory.innerHTML;
    console.log (orders);
    for (var r = 0; r < orders.length; r++) {
        ...
        var header = "<tr><th>Order#: " + order.id + "</th>" +
            ...
            "<th>Details</th></tr>"
        vendors = order.itemSummary;
        var vendorStr = "";

        for (var m = 0; m < vendors.length; m++) {
            vendor = vendors[m];
